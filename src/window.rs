...
fn work_area() -> Result<WorkArea> {
  let output = Command::new("wmctrl").args(["-d"]).output().context("failed to run wmctrl -d")?;

  let text = String::from_utf8_lossy(&output.stdout);
  // Find the current desktop (marked with *)
  let line = text.lines().find(|l| l.contains(" * ")).context("no active desktop found")?;
  ...
  let origin: Vec<i32> =
    parts[0].split(',').map(|s| s.parse()).collect::<std::result::Result<_, _>>()?;
  let size: Vec<i32> =
    parts[1].split('x').map(|s| s.parse()).collect::<std::result::Result<_, _>>()?;

  Ok(WorkArea { x: origin[0], y: origin[1], w: size[0], h: size[1] })
}
...
fn frame_extents(window_id: &str) -> FrameExtents {
  let output = Command::new("xprop").args(["-id", window_id, "_GTK_FRAME_EXTENTS"]).output();
  ...
  if let Ok(output) = output {
    ...
    if let Some(pos) = text.find("= ") {
      let vals: Vec<i32> =
        text[pos + 2..].split(',').filter_map(|s| s.trim().parse().ok()).collect();
      if vals.len() == 4 {
        return FrameExtents { left: vals[0], right: vals[1], top: vals[2], bottom: vals[3] };
      }
    }
  }
...
fn window_id_by_title(title: &str) -> Result<String> {
  ...
  loop {
    ...
    if let Some(first_line) = text.lines().next()
      && let Ok(dec_id) = first_line.trim().parse::<u64>()
    {
...
fn place_window(
  selector: &str, wid: &str, wa: &WorkArea, wa_x: i32, wa_y: i32, wa_w: i32, wa_h: i32,
) -> Result<()> {
